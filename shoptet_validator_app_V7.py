...
DEFAULT_PROFILE = {
    ...
    "params": {"repeat_element": "", "classifier_field": "", "fname_field": "", "fvalue_field": "",
    ...
    "price_adjust": {"discount_percent": 0, "add_vat": False, "vat_percent": "21", "order": "discount_first"},
}
...
def classifier_field_candidates(root, item_tag, repeat_tag, sample_size=30):
    """Mezi podřazenými poli opakujícího se elementu (repeat_tag) najde ty, které
    mají MÁLO různých hodnot vůči počtu výskytů - typický 'rozlišovací' tag."""
        ...
        for feat in item.xpath(f".//*[local-name()='{repeat_tag}']"):
            for child in feat:
                ...
                if child.text and child.text.strip():
                    field_values.setdefault(fname, []).append(child.text.strip())
    scored = []
    for f, vals in field_values.items():
        distinct = len(set(vals))
        if 1 < distinct <= max(20, len(vals) * 0.3):
            scored.append((f, distinct, len(vals)))
    scored.sort(key=lambda x: x[1])
    return [f for f, _, _ in scored]


def get_classifier_values(root, item_tag, repeat_tag, classifier_field, sample_size=200):
    items = root.xpath(f"//*[local-name()='{item_tag}']")[:sample_size]
    counts = {}
    for item in items:
        for feat in item.xpath(f".//*[local-name()='{repeat_tag}']"):
    ...
    for src in items_src:
        ...
        if pcfg2["repeat_element"] and pcfg2["fname_field"] and pcfg2["fvalue_field"]:
            for feat in src.xpath(f".//*[local-name()='{pcfg2['repeat_element']}']"):
                cls_val = gtext(feat, pcfg2["classifier_field"]) if pcfg2["classifier_field"] else None
                ...
                if bucket == "marketing" and (fval or fname):
                    marketing_bullets.append(fval or fname)
                elif bucket == "technical" and fname and fval:
                    technical_pairs.append((fname, fval, funit))
                elif bucket == "size" and fname and fval:
                    size_pairs.append((fname, fval, funit))
...
def apply_price_adjustment(shop_root, cfg):
    ...
    Postup je vždy: 1) k základní ceně se (volitelně) připočte DPH a zaokrouhlí na CELÉ číslo,
    2) na tuto cenu s DPH se (volitelně) uplatní naše sleva, výsledek se opět zaokrouhlí na celé číslo.
    Pokud je sleva použitá, cena PŘED slevou (ale už s DPH) jde do <STANDARD_PRICE> (přeškrtnutá cena
    v Shoptetu) a cena PO slevě do <PRICE>. Bez slevy zůstává jen <PRICE>.
    ...
    for item in shop_root.xpath("./*[local-name()='SHOPITEM']"):
        ...
        ndigits = 0 if add_vat else 2
        list_price = round(p * (1 + vat / 100.0), ndigits) if add_vat else p
        final_price = round(list_price * (1 - discount / 100.0), ndigits) if discount else list_price

        def fmt(v):
            return str(int(v)) if add_vat else f"{v:.2f}"

        price_el[0].text = fmt(final_price)
        if discount:
            std_el = item.xpath("./*[local-name()='STANDARD_PRICE']")
            if std_el:
                std_el[0].text = fmt(list_price)
            else:
                etree.SubElement(item, "STANDARD_PRICE").text = fmt(list_price)
        if add_vat:
...
# =============================================================================
    ...
    if not sample_items:
        ...
        if pcfg["repeat_element"]:
            ...
            classifier_options = ["-- neklasifikovat (nic negenerovat) --"] + suggested + \
                [f for f in field_options[1:] if f not in suggested]
            cur_cls = pcfg.get("classifier_field", "")
            idx = classifier_options.index(cur_cls) if cur_cls in classifier_options else 0
            chosen_cls = st.selectbox(
                "Rozlišovací pole (podle čeho appka pozná druh parametru) - navržené kandidáty jsou nahoře",
                classifier_options, index=idx,
            )
            pcfg["classifier_field"] = "" if chosen_cls.startswith("-- ") else chosen_cls

            if pcfg["classifier_field"]:
                values = get_classifier_values(root, item_tag, pcfg["repeat_element"], pcfg["classifier_field"])
...
# =============================================================================
    ...
    with st.expander("💰 Hromadná úprava ceny (NAŠE SLEVA + DPH)", expanded=False):
        st.caption(
            "Postup: k ceně se (volitelně) připočte DPH a zaokrouhlí na celé číslo. Pokud je zadaná "
            "i naše sleva, tahle cena s DPH (před slevou) jde do <STANDARD_PRICE> (přeškrtnutá cena "
            "v Shoptetu) a cena po naší slevě jde do <PRICE>."
        )
        ...
        with c2:
            pa["add_vat"] = st.checkbox("Přidat DPH k ceně (zaokrouhlí na celé číslo)", value=pa.get("add_vat", False))
            if pa["add_vat"]:
        ...
        if pa["discount_percent"] or pa["add_vat"]:
            ...
            if first_price:
                ...
                if first_standard:
                    line += f" (původní/přeškrtnutá cena: ~~{first_standard} {prof.get('currency', '')}~~)"
                st.caption(line)
