...
def apply_price_adjustment(shop_root, cfg):
    """Hromadná úprava ceny - sleva dodavatele a DPH, v nastavitelném pořadí.
    ...
    order = cfg.get("order", "discount_first")
    try:
    ...
    for item in shop_root.xpath("./*[local-name()='SHOPITEM']"):
        ...
        if order == "vat_first":
            if add_vat:
                p = round(p * (1 + vat / 100.0), 2)
            if discount:
                p = round(p * (1 - discount / 100.0), 2)
        else:
            if discount:
                p = round(p * (1 - discount / 100.0), 2)
            if add_vat:
                p = round(p * (1 + vat / 100.0), 2)
        price_el[0].text = f"{p:.2f}"
        if add_vat:
...
# =============================================================================
    ...
    with st.expander("💰 Hromadná úprava ceny (sleva dodavatele + DPH)", expanded=False):
        pa = prof.get("price_adjust", {"discount_percent": 0, "add_vat": False, "vat_percent": "21", "order": "discount_first"})
        c1, c2 = st.columns(2)
        with c1:
            pa["discount_percent"] = st.number_input(
                "Sleva dodavatele (%) - odečte se od ceny", min_value=0.0, max_value=100.0,
                value=float(pa.get("discount_percent", 0) or 0), step=0.5,
            )
        with c2:
            pa["add_vat"] = st.checkbox("Přidat DPH k ceně", value=pa.get("add_vat", False))
            if pa["add_vat"]:
        ...
        if pa["discount_percent"] and pa["add_vat"]:
            order_choice = st.radio(
                "Pořadí výpočtu",
                ["Nejdřív sleva, pak DPH (sleva se počítá z ceny bez DPH)",
                 "Nejdřív DPH, pak sleva (sleva se počítá z ceny s DPH)"],
                index=0 if pa.get("order", "discount_first") == "discount_first" else 1,
            )
            pa["order"] = "discount_first" if order_choice.startswith("Nejdřív sleva") else "vat_first"
        else:
            pa["order"] = pa.get("order", "discount_first")
        ...
        if pa["discount_percent"] or pa["add_vat"]:
            ...
            if first_price:
                st.caption(f"Náhled ceny prvního produktu po úpravě: **{first_price} {prof.get('currency', '')}**")
