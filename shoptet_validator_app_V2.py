...
SHOPTET_DICT = {
    ...
    "PARAMETERS": {"desc": "Obalovací element parametrů typu velikost (např. u oblečení/obuvi). Pro běžné technické parametry se NEPOUŽÍVÁ.", "mandatory": False},
    "PARAMETER": {"desc": "Jeden parametr velikosti (velikost oblečení/obuvi), obsahuje NAME a VALUE.", "mandatory": False},
    "INFORMATION_PARAMETERS": {"desc": "Obalovací element obecných/technických parametrů produktu (specifikace, rozměry, výkon apod.).", "mandatory": False},
    "INFORMATION_PARAMETER": {"desc": "Jeden obecný/technický parametr produktu, obsahuje NAME a VALUE.", "mandatory": False},
    "VALUE": {"desc": "Hodnota parametru uvnitř <PARAMETER> nebo <INFORMATION_PARAMETER>.", "mandatory": False},
...
SHOPITEM_TAG_ORDER = [
    ...
    "IMAGES", "PARAMETERS", "INFORMATION_PARAMETERS", "LOGISTIC", "STOCK", "VISIBILITY", "URL",
]
...
def convert_bmecat_to_shoptet(root, options):
    ...
    for art in articles:
        ...
        # parametry z technických FEATURE -> INFORMATION_PARAMETERS (ne PARAMETERS!)
        # <PARAMETERS>/<PARAMETER> je v Shoptetu vyhrazeno pro velikosti (oblečení/obuv),
        # obecné technické parametry patří do <INFORMATION_PARAMETERS>/<INFORMATION_PARAMETER>.
    ...
    if options["generate_parameters"]:
        log.append("Technické parametry (FNAME/FVALUE/FUNIT) převedeny do <INFORMATION_PARAMETERS><INFORMATION_PARAMETER><NAME>/<VALUE> (jednotka sloučena do hodnoty). ⚠️ Doporučeno ručně zkontrolovat v tabu 3.")
    if n_img:
...
with st.sidebar:
    ...
    generate_parameters = st.checkbox(
        "Generovat INFORMATION_PARAMETERS z technických FEATURE (doporučeno nechat vypnuté a doplnit ručně v tabu 3)",
        value=False,
    )
...
if st.session_state.shop_root is not None:
    ...
    with tab4:
        ...
        template = st.text_input(
            ...
            value="https://TVUJ-HOSTING.cz/bosch-obrazky/",
            help="Stačí zadat URL složky, kde máš obrázky nahrané pod jejich původními názvy "
                 "(přesně tak, jak jsou v ZIPu z Bosch Prime Poolu). Appka k ní jen připojí "
                 "název souboru z feedu - žádné přejmenovávání.",
        )
        if st.button("Aplikovat URL složky na všechny obrázky bez URL", type="primary"):
            from urllib.parse import quote
            base = template if template.endswith("/") else template + "/"
            cnt = 0
            for el in image_els:
