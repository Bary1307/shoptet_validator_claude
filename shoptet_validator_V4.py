...
# =============================================================================
    ...
    if not sample_items:
        ...
        if pcfg["repeat_element"]:
            field_options = descendant_tags(sample_items[0].xpath(f".//*[local-name()='{pcfg['repeat_element']}']")[0]) \
                if sample_items[0].xpath(f".//*[local-name()='{pcfg['repeat_element']}']") else []
...
# =============================================================================
    ...
    if sample_items:
        ...
        avail_tags = descendant_tags(sample_items[0])
        tag_options = ["-- nevybráno --"] + avail_tags
    ...
    base_url = st.text_input(
        "URL složky ve Shoptet Správci souborů (appka připojí originální název souboru)",
        value=icfg.get("public_base_url") or "https://TVUJ-ESHOP.cz/user/documents/obrazky/",
    )
